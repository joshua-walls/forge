...
| Strict mode | Off | Raises selected shape validation severities |
| Lint run retention | `20` | Number of lint run notes to keep |
| Lint file links | Off | Writes file paths as Obsidian links in reports |
| Lint inline metadata | On | Validates `key:: value` metadata against `inline_fields` |
| Enable stale note review | Off | Flags notes whose review cycle has elapsed |
| Cycle field | `review_cycle` | Field containing review cadence |
| Updated field | `updated` | Field containing last updated date |
| Filter field | `status` | Optional field limiting stale review scope |
| In-scope values | Empty | Values included in stale review |
...
| Inbox retention days | `30` | Age cutoff for stale inbox review |
