...
| `stale_inbox_note` | Inbox note is older than the configured retention threshold when inbox retention action is set to warning | Warning |
...
```text
Inbox retention action → Warn in Vault Lint
```
